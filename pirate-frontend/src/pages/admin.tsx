...
export default class Home extends React.Component<IHomeProps, IHomeState> {
  ...
  async GetDataRequest() {
    ...
    const response = await fetch('http://192.168.1.43:3100/api/GetData', this.iGetDataRequest);
    ...
    if (
      ...
      //Obstacles
      if (this.state.GameInfo?.Obstacles != data.Obstacles) {
        data.Obstacles?.forEach( tr =>{
          this.addItemToMap(tr.location!, tr.type!)
        })
      }
  ...
  async SendDataRequest(GameInfo: IGameInfo) {
    ...
    const response = await fetch('http://192.168.1.43:3100/api/Change', requestOptions);
